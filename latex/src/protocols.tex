...
Finally, all of the ingredients for the reconciliation mechanism are in place. The closest vector any $\mathbf{x}\in\mathbb{R}$ can be computed using the $CVP_{\tilde{D}_4}$ algorithm, descirbed in Algorithm \ref{alg:nh:CVP}, and with this the r-bit reconciliation function \verb|HelpRec| can be defined as 
\begin{equation}
